...
const EditBlogPage = () => {
  ...
  useEffect(() => {
    const fetchBlogData = async () => {
      try {
        const response = await fetch(`/api/blogs/get-blog/${slug}`);
        ...
        const createdAt = new Date(data.blog.createdAt.seconds * 1000);
        ...
        setFormData({
          ...
          viewCount: data.blog.viewCount,
          seo: {
            title: data.blog.seo.title,
            description: data.blog.seo.description,
            keywords: Array.isArray(data.blog.seo.keywords)
              ? data.blog.seo.keywords.join(", ")
              : data.blog.seo.keywords,
            ogImageUrl: data.blog.seo.ogImageUrl,
            metaRobots: data.blog.seo.metaRobots,
            imageTitle: data.blog.seo.imageTitle,
            altText: data.blog.seo.altText,
          },
        });
        if (data.blog.category) {
          const savedCategories = JSON.parse(localStorage.getItem("blogCategories")) || [];
          const updatedCategories = Array.from(
            new Set([...categories, ...savedCategories, data.blog.category])
          );
          localStorage.setItem("blogCategories", JSON.stringify(updatedCategories));
          setCategories(updatedCategories);
        }
        if (data.blog.imageUrl) setImagePreview(data.blog.imageUrl);
        if (data.blog.seo.ogImageUrl)
          setOgImagePreview(data.blog.seo.ogImageUrl);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/blogs/update-blog/${slug}`, {
        method: "PUT",
        headers: {
