...
const EditPortfolioPage = () => {
    ...
    const fetchPortfolioData = async () => {
        try {
            setFetching(true);
            const response = await fetch(`/api/portfolio/get-portfolio/${slug}`);
            const data = await response.json();

            if (data.success) {
                const portfolio = data.portfolio;

                const startDate = portfolio.startDate?.seconds
                    ? new Date(portfolio.startDate.seconds * 1000)
                    : new Date();

                const completedAt = portfolio.completedAt?.seconds
                    ? new Date(portfolio.completedAt.seconds * 1000)
                    : new Date();

                const categoryNames = Object.keys(portfolio.categories || {});
                const mappedCategories = categoryNames.map(name => {
                ...
                if (portfolio.category) {
                    const savedCategories = JSON.parse(localStorage.getItem("portfolioCategories")) || [];
                    const updatedCategories = Array.from(
                        new Set([...localCategories, ...savedCategories, portfolio.category])
                    );
                    localStorage.setItem("portfolioCategories", JSON.stringify(updatedCategories));
                    setLocalCategories(updatedCategories);
                }
                ...
                if (portfolio.image) {
                    setMainImagePreview(portfolio.image);
                }

                if (portfolio.seo?.ogImageUrl) {
                    setOgImagePreview(portfolio.seo.ogImageUrl);
                }
            } else {
    ...
    const handleOgImageChange = async (e) => {
        ...
        try {
            if (portfolioData.seo.ogImageUrl) {
                await deleteImageFromFirebase(portfolioData.seo.ogImageUrl);
            }
    ...
    const handleMainImageChange = async (e) => {
        ...
        try {
            if (portfolioData.image) {
                await deleteImageFromFirebase(portfolioData.image);
            }
    ...
    const removeImage = async (categoryName, imageId) => {
        const category = portfolioData.categories[categoryName];
        const imageToDelete = category.images.find(img => img.id === imageId);

        if (imageToDelete && imageToDelete.url) {
            await deleteImageFromFirebase(imageToDelete.url);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (categories.length === 0) {
            alert("Please add at least one category before updating the portfolio.");
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`/api/portfolio/update-portfolio/${slug}`, {
                method: "PUT",
                headers: {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <div className="max-w-7xl mx-auto px-4 py-8">
                {/* Header */}
                <div className="mb-8">
                    <h1 className="text-4xl font-bold text-gray-900 mb-2">Edit Portfolio Project</h1>
                    <p className="text-gray-600">Update your work across different categories</p>
                </div>
